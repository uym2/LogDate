...
ggplot(d,aes(x=clockModel,y=value,group=interaction(method,clockModel),fill=method)) +
...
ggplot(d,aes(x=method,y=value,group=method,fill=method)) +
  geom_bar(stat="summary") + geom_errorbar(stat="summary",width=0.4) +
  theme_bw() + facet_wrap(~clockModel) + ylab("rmse") +
